...
if __name__ == '__main__':
    problems = ['pla33810_n33809']#, 'a280_n2790','a280_n1395', 'fnl4461_n4460', 'fnl4461_n22300', 'fnl4461_n44600','pla33810_n33809', 'pla33810_n169045', 'pla33810_n338090']
    ...
    for problem in problems:
        ...
        for i in range(10):
            ...
            tsp_base = profit(ttsp_permutation_original, np.zeros(ttsp.item_num), ttsp)
            print(tsp_base)
            test_case = TestCase(17000, 0.005, ttsp)
            n = ttsp.dim
            p = 3
            value, rent = profit(ttsp_permutation_original, np.zeros(ttsp.item_num), ttsp, seperate_value_rent=True)
            # save_result(route, knapsack, problem, prof, fact)
            print(value - ttsp.renting_ratio * rent)
            print('rent:')
            print(rent)
            ea = OnePlusOneEA(ttsp, ttsp_permutation_original, np.zeros(ttsp.item_num), test_case, lambda n: return_bin_vals(n, p / n),
                              rent, 42)
            ea_profit, ea_kp, ea_tour, test_c = ea.optimize()
            #while fact < 7:
            fact = 1.3
            while fact < 3.7:
                greedy = greedy_ttsp(ttsp, ea_tour)
                ...
                route, knapsack, prof = run_greedy(ttsp, route, 1000, fact)
                save_result(route, knapsack, problem, prof, fact)
                route, knapsack, prof = run_greedy(ttsp, reversePerm(route), 1000, fact)
                save_result(reversePerm(route), knapsack, problem, prof, fact, 'rev')
                fact += 0.3
